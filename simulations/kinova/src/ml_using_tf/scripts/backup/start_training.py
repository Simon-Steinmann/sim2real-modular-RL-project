...
if __name__ == '__main__':
    ...
    env = gym.make('j2n6s300Test-v1')
    rospy.loginfo ( "Gym environment done")

    # Set the logging system
    rospack = rospkg.RosPack()
    pkg_path = rospack.get_path('ml_using_tf')
    ...
    Alpha = rospy.get_param("/ml_using_tf/alpha")
    Epsilon = rospy.get_param("/ml_using_tf/epsilon")
    Gamma = rospy.get_param("/ml_using_tf/gamma")
    epsilon_discount = rospy.get_param("/ml_using_tf/epsilon_discount")
    nepisodes = rospy.get_param("/ml_using_tf/nepisodes")
    nsteps = rospy.get_param("/ml_using_tf/nsteps")

    running_step = rospy.get_param("/ml_using_tf/running_step")
