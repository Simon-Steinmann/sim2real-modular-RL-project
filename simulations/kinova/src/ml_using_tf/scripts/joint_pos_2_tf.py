...
class Pos2Tf(object):
    def __init__(self):   
        ...
        self.end_effector_trans = TransformStamped()
        self.end_effector_trans.header.frame_id = "j2n6s300_link_6"

        self.end_effector_trans.child_frame_id = 'j2n6s300_joint_end_effector'
        self.end_effector_trans.transform.translation.z = -0.16
        self.end_effector_trans.transform.rotation.x =  0.7071067811882787   
        self.end_effector_trans.transform.rotation.y =  0.7071067811848163    
        self.end_effector_trans_mat=numpify(self.end_effector_trans.transform)
        ...
        #print(self.end_effector_trans_mat)
    ...
    def calculate_joints(self, position, num_joints):        
        ...
        T_c = [np.identity(4)]
        tf_msg = self.urdf_transforms[:num_joints] 
        ...
        if num_joints >= 6:
            end_effector_mat = np.dot(T_c[6], self.end_effector_trans_mat)    
            self.end_effector_trans.transform = msgify(Transform, end_effector_mat)
            tf_msg.append(self.end_effector_trans)
        return tf_msg
...
if __name__ == "__main__":
    ...
    for i in range(10):
        print(obj.calculate_joints([0.0, 2.9, 1.3, -2.07, 1.4, 0.0, 0, 0, 0, 0, 0, 0.0] ,6))
    rospy.spin()
