...
class j2n6s300_Environment(gym.Env):
    ...
    def __init__(self, proxyID='Env1', deterministic=False):
        print(proxyID)
        self.env = ProxyGymEnv(proxyID)
        self.action_space = spaces.Box(-np.full((6),1) , np.full((6),1))
        self.observation_space = spaces.Dict(dict(
            desired_goal=spaces.Box(-np.full((3),2) , np.full((3),2)),
            achieved_goal=spaces.Box(-np.full((3),2) , np.full((3),2)),
            observation=spaces.Box(-np.full((56),2) , np.full((56),2)),
        ))

        self.seed()
        self.reward_threshold = 325
        self.trials = 10
    ...
    def step(self, action):
        ...
        if self.done:
            if self.n_step <= self.max_episode_steps:
                self.reward = -500 + (self.n_step -self.max_episode_steps)*25
